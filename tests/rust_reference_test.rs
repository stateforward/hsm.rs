...
async fn test_path_resolution_patterns() -> Result<()> {
    ...
    let model = define!(
        ...
        state!(
            "parent",
            state!("child1", transition!(on!("next"), target!("../child2"))),
            state!(
                "child2",
...
async fn test_hierarchical_patterns() -> Result<()> {
    ...
    let model = define!(
        ...
        state!(
            ...
            state!(
                "running",
                state!("idle", transition!(on!("start"), target!("../processing"))),
                state!(
                    "processing",
            ...
            state!("error", transition!(on!("reset"), target!("../running")))
        )
    );
...
async fn test_error_handling_patterns() -> Result<()> {
    ...
    let model = define!(
        ...
        state!("normal", transition!(on!("error"), target!("../error"))),
        state!("error")
    );
...
async fn test_validation_errors() -> Result<()> {
    ...
    let bad_model = define!(
        "BadChoiceMachine",
        initial!(target!("start")),
        state!("start", transition!(on!("go"), target!("../bad_choice"))),
        choice!(
            "bad_choice",
            transition!(guard!(counter_guard), target!("never")) // Missing guardless fallback!
        ),
        state!("never")
    );
...
async fn test_deferred_events() -> Result<()> {
    ...
    let model = define!(
        ...
        state!("ready", transition!(on!("deferred_event"), target!(".")))
    );
...
async fn test_function_signatures() {
    ...
    let _entry: fn(&Context, &mut MyInstance, &Event) -> Pin<Box<dyn Future<Output = ()> + Send>> =
        state_entry;
    let _exit: fn(&Context, &mut MyInstance, &Event) -> Pin<Box<dyn Future<Output = ()> + Send>> =
        state_exit;
    let _effect: fn(&Context, &mut MyInstance, &Event) -> Pin<Box<dyn Future<Output = ()> + Send>> =
        transition_effect;
    let _activity: fn(
        &Context,
        &mut MyInstance,
        &Event,
    ) -> Pin<Box<dyn Future<Output = ()> + Send>> = background_activity;
...
async fn test_async_runtime_integration() -> Result<()> {
    ...
    let model = define!(
        "AsyncMachine",
        initial!(target!("start")),
        state!("start", transition!(on!("go"), target!("../processing"))),
        state!(
            "processing",
            activity!(background_activity),
