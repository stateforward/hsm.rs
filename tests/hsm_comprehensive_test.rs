...
async fn test_basic_state_machine_creation() -> Result<()> {
    ...
    let model = define("TestMachine", vec![
        state("idle"),
        state("running"),
        initial(),
    ]);
...
async fn test_simple_transitions() -> Result<()> {
    ...
    let model = define("SimpleTransitionMachine", vec![
        state("start"),
        state("end"),
        initial(),
    ]);
...
async fn test_entry_and_exit_actions() -> Result<()> {
    ...
    let model = define("ActionMachine", vec![
        state("active"),
        initial(),
    ]);
...
async fn test_guard_conditions() -> Result<()> {
    ...
    let model = define("GuardMachine", vec![
        state("testing"),
        state("success"),
        state("failure"),
        initial(),
...
async fn test_choice_pseudostates() -> Result<()> {
    ...
    let model = define("ChoiceMachine", vec![
        state("deciding"),
        choice("junction"),
        state("option1"),
        state("option2"),
...
async fn test_final_states() -> Result<()> {
    ...
    let model = define("FinalMachine", vec![
        state("working"),
...
async fn test_activities() -> Result<()> {
    ...
    let model = define("ActivityMachine", vec![
        state("active"),
        state("inactive"),
        initial(),
    ]);
...
async fn test_hierarchical_states() -> Result<()> {
    ...
    let model = define("HierarchicalMachine", vec![
        state("parent"),
        state("child1"),
        state("child2"),
        initial(),
...
async fn test_event_dispatching() -> Result<()> {
    ...
    let model = define("EventMachine", vec![
        state("listening"),
        state("responding"),
        initial(),
    ]);
...
async fn test_deferred_events() -> Result<()> {
    ...
    let model = define("DeferredMachine", vec![
        state("busy"),
        state("ready"),
        initial(),
    ]);
...
async fn test_validation() -> Result<()> {
    ...
    let model = define("ValidationMachine", vec![
        state("valid"),
        initial(),
    ]);
...
async fn test_context_cancellation() -> Result<()> {
    ...
    let model = define("CancellationMachine", vec![
        state("running"),
        initial(),
    ]);
...
async fn test_error_handling() -> Result<()> {
    ...
    let model = define("ErrorMachine", vec![
        state("normal"),
        state("error"),
        initial(),
    ]);
...
async fn test_performance_with_many_events() -> Result<()> {
    ...
    let model = define("PerformanceMachine", vec![
        state("processing"),
        initial(),
    ]);
    ...
    for i in 0..100 {
        hsm.dispatch(&ctx, Event::new(&format!("event_{}", i))).await?;
    }
...
async fn test_concurrent_access() -> Result<()> {
    ...
    let model = define("ConcurrentMachine", vec![
        state("shared"),
        initial(),
    ]);
    ...
    let handle = tokio::spawn(async move {
        let _ = hsm_clone.dispatch(&ctx_clone, Event::new("concurrent_event")).await;
    });
...
async fn test_memory_management() -> Result<()> {
    ...
    let model = define("MemoryMachine", vec![
        state("allocating"),
        state("deallocating"),
        initial(),
    ]);
...
async fn test_event_data_handling() -> Result<()> {
    ...
    let model = define("EventDataMachine", vec![
        state("processing"),
        initial(),
    ]);
...
async fn test_path_resolution() -> Result<()> {
    ...
    let model = define("PathMachine", vec![
        state("root"),
        state("child"),
        initial(),
    ]);
...
async fn test_queue_behavior() -> Result<()> {
    ...
    let model = define("QueueMachine", vec![
        state("receiver"),
        initial(),
    ]);
...
async fn test_model_lifecycle() -> Result<()> {
    ...
    let model = define("LifecycleMachine", vec![
        state("start"),
        state("middle"),
        state("end"),
        initial(),
