...
fn basic_entry(_ctx: &Context, inst: &mut BasicTestInstance, _event: &Event) -> Pin<Box<dyn Future<Output = ()> + Send>> {
    inst.log_action("basic-entry");
    inst.increment();
    Box::pin(async move {})
}

fn basic_exit(_ctx: &Context, inst: &mut BasicTestInstance, _event: &Event) -> Pin<Box<dyn Future<Output = ()> + Send>> {
    inst.log_action("basic-exit");
    Box::pin(async move {})
}

fn transition_effect(_ctx: &Context, inst: &mut BasicTestInstance, _event: &Event) -> Pin<Box<dyn Future<Output = ()> + Send>> {
    inst.log_action("transition-effect");
    Box::pin(async move {})
}

fn counter_effect(_ctx: &Context, inst: &mut BasicTestInstance, _event: &Event) -> Pin<Box<dyn Future<Output = ()> + Send>> {
    inst.increment();
...
async fn test_state_machine_lifecycle_with_entry_exit() -> Result<()> {
    ...
    let model = define!("LifecycleMachine",
    ...
    let inst = inst_ref.as_any().downcast_ref::<BasicTestInstance>().unwrap();
...
async fn test_multiple_transitions_from_same_state() -> Result<()> {
    ...
    let model = define!("MultiTransitionMachine",
        initial!(target!("idle")),
        state!("idle",
        ...
        state!("state1",
            transition!(on!("back"), target!("../idle"))
        ),
        state!("state2",
            transition!(on!("back"), target!("../idle"))
        )
...
async fn test_unknown_events_ignored() -> Result<()> {
    ...
    let model = define!("UnknownEventMachine",
...
async fn test_event_with_data() -> Result<()> {
    ...
    fn data_effect(_ctx: &Context, inst: &mut BasicTestInstance, event: &Event) -> Pin<Box<dyn Future<Output = ()> + Send>> {
    ...
    let model = define!("EventTypeMachine",
...
async fn test_context_cancellation() -> Result<()> {
    ...
    fn cancellation_activity(_ctx: &Context, inst: &mut BasicTestInstance, _event: &Event) -> Pin<Box<dyn Future<Output = ()> + Send>> {
    ...
    let model = define!("CancellationMachine",
        initial!(target!("active")),
        state!("active",
