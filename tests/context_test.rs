...
fn test_context_clone() {
    ...
    // But they should be independent after cloning
    let ctx3 = Context::new();
    let ctx4 = ctx3.clone();
    
    ctx3.cancel();
    assert!(ctx3.is_done());
    assert!(!ctx4.is_done(), "Cloned context should be independent");
}
...
fn test_context_memory_ordering() {
    ...
    let reader = thread::spawn(move || {
        ...
        assert!(data_written_clone.load(Ordering::Acquire),
...
fn test_context_use_patterns() {
    ...
    assert_eq!(iterations, 0, "Should exit immediately when context is done");
...
fn test_context_debug_impl() {
    ...
    assert!(debug_str.contains("Context"), "Debug should mention Context");
